...
const nerdjokes_url = "https://pixelated.tech/nerdjokes.html" ;
const log = true ;
...
function getTeamId(event, env){
    ...
    else if(event.team) {  team_id = event.team.id ; }
    else { team_id = "." }
...
async function logEvent(event, data){
    ...
    var logged = await awstools.writeToDynamo({
        tablename: awstools.jokesLogDB,
        item: {
            team_id : data.team_id ,
            log_id: Date.now().valueOf().toString() ,
            channel_id : channelId ,
            event_type : eventType ,  //image upload, image share 
            user_id : userId
        }
    });
...
async function getFileInfo(file_id, b_token){
    ...
    return new Promise(function(resolve, reject) {
        const request = HTTPS.request(options, function(res) {
            ...
            res.on("end", function () {
                if(log) console.log("FILE INFO - Response Results", retJSON.toString());
                resolve(JSON.parse(retJSON));
            });
        });
        request.on("error", function (error) { 
            console.log("FILE INFO - ERROR : ", JSON.stringify(error));
            console.log("FILE INFO - ERROR Code : ", JSON.stringify(error.code));
            console.log("FILE INFO - ERROR Message : ", JSON.stringify(error.message));
            console.log("FILE INFO - ERROR Stack : ", JSON.stringify(error.stack));
            reject(error);
        });
        request.end();
...
async function processApiMsg(data) {
    ...
    (data.response_type) ? message.response_type = data.response_type : message.response_type = 'in_channel' ; // 'ephemeral'
    (data.thread_ts) ? message.thread_ts = data.thread_ts : null ;
    (data.return_im) ? message.return_im = data.return_im : null ;
    ...
    (data.trigger_id) ? msgData.trigger_id = data.trigger_id : null ; // HERE?
...
async function slackApiPost(message, data){
    // ==#####== SLACK API POST ==#####===
    if(log) console.log("SLACK API POST - Message : ", message);
...
async function getTimeZoneOptions(){
    ...
    for (var i = 0; i < timezones.length; i++) {
        var item = {};
        // item.text = timezones[i].value;
        // item.value = timezones[i].offset;
        // var item = { text: timezones[i].value , value: timezones[i].offset };
